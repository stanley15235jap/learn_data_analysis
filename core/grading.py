...
def submit_quiz_answer(unit_id: str, question, user_answer) -> bool:
    """批改一題、寫入 quiz_attempts、同步更新該 concept 的弱點狀態。回傳是否答對。"""
    is_correct = grade_answer(question, user_answer)
    conn = get_conn()
    insert_quiz_attempt(conn, unit_id, question.id, question.concept_id, is_correct, user_answer)
    update_weakness(conn, question.concept_id, unit_id, is_correct)
    conn.commit()
    conn.close()
    return is_correct


def submit_exercise(unit_id: str, exercise, student_code: str) -> dict:
    """執行學生程式碼、批改、寫入 exercise_submissions。回傳執行結果 dict。"""
    result = run_exercise(student_code, exercise.checker_code)
    insert_exercise_submission(unit_id, exercise.id, student_code, result["passed"], result["message"])
    return result
